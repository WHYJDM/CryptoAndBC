...
describe("MultiSigWallet", function () {
  ...
  describe("Transaction Submission", function () {
    ...
    it("should emit SubmitTransaction event", async function () {
      await expect(multiSig.connect(owner1).submitTransaction(addr1.address, ethers.parseEther("1"), "0x"))
        .to.emit(multiSig, "SubmitTransaction")
        .withArgs(owner1.address, 0, addr1.address, ethers.parseEther("1"), "0x");
    });
  ...
  describe("Deposit", function () {
    ...
    it("should emit Deposit event on Ether receipt", async function () {
      ...
      await expect(owner1.sendTransaction({ to: multiSig.getAddress(), value: depositAmount }))
